...
if(post('addorder')){
    Order::add_quantity_by_carts_product_id(post('addorder'));
    die();
}
if(post('subtractorder')){

    Order::reduce_quantity_by_carts_product_id(post('subtractorder'));
    Order::update_cart_status_zero(post('subtractorder'));
    die();
}
if(post('textarray')){
    $text=post('textarray');
    //damp($text[0]);
    Order::text_quantity_by_carts_product_id($text[0],$text[1]);
    die();
}
if(post('order')){

    Order::add_order_count($customermarket['id']);
    $link="https://qbdigitals.com/customer/new_order?page=1";
    ...
    if ($customer['marketType']==3){
        $getcard=Order::get_card_by_cardid_markettype3($result);
    }
    else{
        $getcard=Order::get_card_by_cardid($result);
    }
...
<table border="1" cellpadding="2" cellspacing="2" nobr="true">
  ...
  <th colspan="1" align="center">Total Price </th>
    ...
    foreach ($getcard as $item) {

        $profits=Customer::get_profit($customer['userId']);
        $product=Products::get_product_detail($item['productId'], $item['currency'], $item['product_origin']);

        $newprofit = [];

        foreach ($profits as $profit){ if($profit["genus_name"]==$item['cins']){ $profit_price= $profit["profit"];} }
        $price = Order::get_product_by_api($item['productId'], $customercurrency['currency'], $item['product_origin']);
        ...
        $totals['sqm']+=(number_format($item['quantity'],2)*number_format($price['totalquan'],2));
        ...
        $totals['price']+=number_format(number_format($price['totalquan'],2)*number_format($roundedPrice,2),2)*$item['quantity'];
        ...
        $tbl.='   <tr>
                  ...
                  <th colspan="1" align="center">'.$price['totalquan']*$item['quantity'].' m2'.'</th>
                  <th colspan="1" align="center">'. str_replace('KT', 'BOX', $price['pkstext']) .'</th>
                       <th colspan="1" align="center">'.$item['quantity'].'</th>
                  <th colspan="1" align="center">'.number_format(($roundedPrice ),2).'</th>
                 
                  <th colspan="1" align="center">'.number_format(number_format(number_format($price['totalquan'],2)*number_format($roundedPrice,2),2)*$item['quantity'],2).'</th>
                 </tr>';
    }
...
<table border="1" style="padding: 5px" nobr="true" >
 ...
 <b>'.$rowsicilno.'</b>'.$sicilno.' <b>'.$rowmersisno.'</b>'.$mersisno.'
   ...
   <td colspan="1" align="center">'.$totals['palletPcs'].'<br/>'.$totals['sqm'].'<br/><br/>'.$grossweight.' KGS<br/>'.$netweight.' KGS<br/>Open Account<br/><br/></td>
    ...
    $result=Order::carts_order(post('deliveryType'));
